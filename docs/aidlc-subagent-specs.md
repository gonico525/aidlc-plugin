...
| パラメータ | 値 | 説明 |
|-----------|-----|------|
| flavor | DDD / BDD / ストーリーベース | テスト導出方法を決定 |
...
## E2E ブラウザ検証（UIプロジェクトの場合）
※ E2E検証はstep6-finalで手動実施する。このセクションはテンプレートとして生成し、
人間が検証結果を記入する。非UIプロジェクトの場合は「該当なし」と記載する。
- 検証方法: {手動 / 該当なし}
- 検証結果: {全件OK / NG件数 / 該当なし}
```
...
| ADR_DEFERRAL_RESOLVED | ADR延期の解決状況（全延期が解決済みか） | 構築(完了モード) |
