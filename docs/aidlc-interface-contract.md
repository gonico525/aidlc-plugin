...
- **並行ボルト実行時:** 各ボルトは独自の git worktree/ブランチ上で manifest.md を更新する。
  成果物IDにボルト識別子（`B{ユニット番号}.{ボルト番号}`）を含めることで行の一意性を保証する。
  統合ブランチへのマージ時に各ブランチの追記行が統合される。
...
**生成タイミング:** build-bolt の完了モード（step6-final）で生成する。
全ボルトのPRが統合ブランチにマージ済みであることが前提条件。
