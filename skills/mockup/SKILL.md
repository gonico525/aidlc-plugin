...
description: >
  ...
  ユーザーストーリーと受け入れ基準を入力として、画面一覧、画面遷移図、
  インタラクティブなUIモックアップ（HTML/React）を生成する。
  ステークホルダーへのデモ、チーム内のUI/UX認識合わせ、
  フロントエンド先行開発の足場として機能する。
  aidlc-user-stories完了後に任意で実行可能（スキップ可能）。
  このスキルは以下の場合にトリガーすべき：ユーザーが「モックアップ」「UI設計」
  「画面設計」「プロトタイプ」「ワイヤーフレーム」「画面遷移」「デモ画面」
  「UIモック」「画面を見せたい」「見た目を先に作りたい」と言及した場合。
  また、aidlc-user-storiesの完了後に「ステークホルダーに見せたい」
  「UIのイメージを固めたい」という文脈でも使用する。
